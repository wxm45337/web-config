...
var date = require('../common/date_format.js'),
    _dirname = require('../common/GLOBAL')._dirname;
  
var DEFAULT = "%s %s -- %s %s HTTP/%s, %s %s";

// 加载配置文件
var log4jsJSON = path.join(_dirname,'config/log4js.json'),
    objConfig = JSON.parse(fs.readFileSync(log4jsJSON, "utf8"));  

  
// 检查配置文件所需的目录是否存在，不存在时创建  
if(objConfig.appenders){  
    var baseDir =  path.join(_dirname,objConfig["customBaseDir"]);  
